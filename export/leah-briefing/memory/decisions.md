...
- **[2026-03-22] Morning briefing — restored and scheduled**
  - Context: Original system was built 2026-03-21 then removed. Restored with PM2 scheduler (no cron/systemd available).
  - Architecture: `scripts/scheduler.mjs` (PM2 daemon) → `scripts/morning-briefing.sh` (bash) → `claude --print` → Slack webhook
  - Schedule: daily 8:00 AM, configurable in scheduler.mjs
  - Requires: `.env` with `SLACK_WEBHOOK_URL` set to a real Slack webhook
